...
* Se creó la clase ReconocedorDeDireccionDelJAR, cuyo fin es obtener la dirección en la que se almacenará la carpeta de salida. Este comportamiento ya existía, pero acoplado con la clase CreadorDeCarpetaDeSalida, lo cual no solamente era un error de diseño sino que generaba un bug: el modo de salida No Output creaba una carpeta nueva y clonaba en ella el contenido de la carpeta "plantilla", lo cual no debía ocurrir.
* Se han actualizado debidamente los diagramas de clase y secuencia incluidos en el trabajo.
* Se ha modificado la estructura del proyecto, separándolo en una serie de paquetes que contienen elementos del dominio y clases necesarias para la operación con los mismos.
---
